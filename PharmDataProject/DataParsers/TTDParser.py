...
class TTDParser:
    ...
    def ttdTxt2Dict_2(self, fin):
        ...
        def read_abbr(tsv_file_name):
            ...
            dic = dict(
                (key, value) for key, value in row_splice)  # Convert the list in the previous row to dictionary form
            return fp, dic
    ...
    def tsv_to_json6(self, tsv_file_name, json_file_name):
        """
    ...
    def toJson(self, dics, fout_json):
        with open(fout_json, 'w') as json_f:
            json.dump(dics, json_f, indent=4)

    def parse(self, cfgfile='../conf/drugkb.config'):
        ...
        for idx in range(4, int(config.get(section, 'col_num')) - 5):
            ...
            if idx == 2:
                dics = [x for x in self.ttdTxt2Dict_2(fin_csv)]

            elif idx == 4:
                self.tsv_to_json6(fin_csv, fout_json)
                ...
                dics = [x for x in self.excel2dict(fin_csv, tables)]
            elif idx == 7:
                dics = [x for x in self.csv2dict(fin_csv, skiprows=15)]
            elif idx in [8, 9]:
                dics = [x for x in self.csv2dict(fin_csv, skiprows=17)]
            else:
                dics = [x for x in self.ttdTxt2Dict(fin_csv)]
            self.toJson(dics, fout_json)
