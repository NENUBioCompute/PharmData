...
def processDemo():
    for filename in os.listdir(directoryPath):
        if "DEMO" in filename.upper() and "TXT" in filename.upper():
            ...
            demo_df = pd.read_csv(directoryPath + "/" + filename, low_memory=False, sep='$', on_bad_lines='warn')
            ...
            try:
                ...
                gndr_cod_map = {'M': "0", 'F': "1", 'UNK': "2"}
                demo_df['gndr_cod'] = demo_df['gndr_cod'].map(gndr_cod_map)
...
def processDrug():
    for filename in os.listdir(directoryPath):
        if "DRUG" in filename.upper() and "TXT" in filename.upper():
            print("Process " + filename)
            drug_df = pd.read_csv(directoryPath + "/" + filename, low_memory=False, sep="$", on_bad_lines='warn')
...
def processReac():
    for filename in os.listdir(directoryPath):
        if "REAC" in filename.upper() and "TXT" in filename.upper():
            print("Process " + filename)
            reac_df = pd.read_csv(directoryPath + "/" + filename, low_memory=False, sep="$", on_bad_lines='warn')
...
def processOutc():
    for filename in os.listdir(directoryPath):
        if "OUTC" in filename.upper() and "TXT" in filename.upper():
            print("Process " + filename)
            outc_df = pd.read_csv(directoryPath + "/" + filename, low_memory=False, sep="$", on_bad_lines='warn')
            ...
            try:
                ...
                outc_df = outc_df[pd.notnull(outc_df['outc_code'])]
                outc_df = outc_df[outc_df['outc_code'].isin(['DE', 'LT', 'HO', 'DS', 'CA', 'RI', 'OT'])]
                outccode_map = {'DE': '0', 'LT': '1', 'HO': '2', 'DS': '3', 'CA': '4', 'RI': '5', 'OT': '6'}
                outc_df['outc_code'] = outc_df['outc_code'].map(outccode_map)
...
def processTher():
    for filename in os.listdir(directoryPath):
        if "THER" in filename.upper() and "TXT" in filename.upper():
            print("Process " + filename)
            ther_df = pd.read_csv(directoryPath + "/" + filename, low_memory=False, sep="$", on_bad_lines='warn')

            # process outc_cod
            try:
                ther_df = ther_df[pd.notnull(outc_df['outc_cod'])]
                ther_df = ther_df[outc_df['outc_cod'].isin(['DE', 'LT', 'HO', 'DS', 'CA', 'RI', 'OT'])]
                outccod_map = {'DE': '0', 'LT': '1', 'HO': '2', 'DS': '3', 'CA': '4', 'RI': '5', 'OT': '6'}
                outc_df['outc_cod'] = outc_df['outc_cod'].map(outccod_map)
            except KeyError:
                outc_df = outc_df[pd.notnull(outc_df['outc_code'])]
...
def processIndi():
    for filename in os.listdir(directoryPath):
        if "INDI" in filename.upper() and "TXT" in filename.upper():
            print("Process " + filename)
            indi_df = pd.read_csv(directoryPath + "/" + filename, low_memory=False, sep="$", on_bad_lines='warn')

            # keep primaryid, caseid, indi_pt


            # process indi_pt
            indi_df = indi_df[pd.notnull(indi_df['indi_pt'])]
            indi_df['indi_pt'] = indi_df['indi_pt'].str.strip().str.lower()  # to lowercase
            indi_df = indi_df[~indi_df['indi_pt'].isin(['unknown'])]  # drop unknown
            indi_df['indi_pt'] = indi_df['indi_pt'].map(
                lambda x: x[:-1] if str(x).endswith(".") else x)  # fix ending with period

            # save file
            indi_df.to_csv(directoryPath + "/" + filename[:-4] + '.csv', header=True, index=False)
