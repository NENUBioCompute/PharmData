...
# 处理每个子列表中的化合物数据并保存到MongoDB
def process_and_save_data():
    # 创建MongoDB客户端和数据库、集合
    client = MongoClient(mongo_url)
    db = client['PharmRG']
    collection = db['PubChem']

    cid_lists = generate_cid_lists()

    for sublist in cid_lists:
        process_compounds(sublist, collection)

    # 关闭MongoDB连接
    client.close()

    print("All compounds processed and saved to MongoDB.")
